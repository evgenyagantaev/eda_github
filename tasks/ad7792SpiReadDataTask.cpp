...
void ad7792SpiReadDataTask(void *pvParameters)
{
   ...
   while(1)
   {
      ...
      /*
      if((!timingResetFlag) && (Conv < 200))
      //if(0)
      {
         // adc in abnormal regime
         adcAbnormal = 1;
      }
      else
      {
         // adc in normal regime
         adcAbnormal = 0;
      }
      
      
      if(((GPIOA->IDR & GPIO_Pin_6) == (uint32_t)Bit_RESET) || timingResetFlag)
      {
         adcFault = 1;
         // no correct interface with ad7792
         // reset the device
         timingResetFlag = 0;
         ad7792.reset();
         // delay for an adc to start conversion sequence
      ...
      volatile int badAdcCounter = 0;
      if(diagnost.badAdcRange)
      {
         vTaskDelay(105);
         badAdcCounter++;
         if(badAdcCounter > 100)
         {
            badAdcCounter = 0;
            ad7792.reset();
